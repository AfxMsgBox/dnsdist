...
所有下载都有超时和最大文件大小限制。任一代理源下载失败或解析后为空，都会中止本次更新。`DNSDIST_DOWNLOAD_PROXY` 非空时，域名更新器通过该 HTTP/HTTPS 代理下载全部来源，并忽略 `NO_PROXY` 绕过规则；变量缺失或为空时显式直连，不继承进程环境中的其他代理设置。
...
独立下载的 `install.sh` 内置最小引导逻辑。它提示安装目录，并在首次 GitHub 下载前从运行中的 Mihomo 进程定位 YAML 配置，按顶层 `mixed-port`、`port` 的顺序识别 HTTP 兼容代理并询问用户是否使用。纯 `socks-port` 不自动采用。确认后的代理通过 `--download-proxy` 传递给完整安装器并保存为 `DNSDIST_DOWNLOAD_PROXY`；用户拒绝时通过 `--no-download-proxy` 防止完整安装阶段重复询问。引导器优先用 `wget`、回退到 `curl` 下载 GitHub `main` 分支压缩包。安装器不会调用软件包管理器；下载工具、dnsdist、WireGuard 工具、Python 3.10+ 等缺失时，它会列出缺失命令和建议的手动安装命令，然后退出。完整压缩包先解压到临时目录，再由其中的完整安装器部署或刷新目标目录。
...
更新器在下载新包前加载当前本机配置，因此 GitHub 压缩包和预检阶段的域名规则使用相同的 `DNSDIST_DOWNLOAD_PROXY`。预检通过后才停止 timer 与 dnsdist，通过同文件系统目录重命名切换已验证的新版本并启动服务。systemd 单元虽然位于临时构建树中，但其中的 `EnvironmentFile`、`ExecStart` 和写入路径必须预先渲染为最终安装目录；渲染后还会拒绝任何残留占位符或临时目录引用，避免切换后静默丢失本机环境参数。这样即使当前生成规则已不兼容，新生成器也能在切换前修复它，并缩短服务停机时间。任一步失败都会先输出 dnsdist 状态与 journal，再把旧目录移回原路径并尝试恢复服务。成功后删除临时旧版本。

### 9.3 卸载边界

卸载器先停止并禁用 timer，停止可能仍在执行的两个 oneshot 更新服务，再停止并禁用 dnsdist。完成服务收敛后，它只移除目标确实指向当前安装目录的系统软链接，并在存在时恢复 dnsdist 软件包原配置。默认保留集中安装目录；`--purge` 才删除整个安装根目录。卸载器不移除系统软件包，也不修改 WireGuard、Mihomo 或防火墙。
