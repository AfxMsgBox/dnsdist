...
独立下载的 `install.sh` 内置最小引导逻辑。它提示安装目录，优先用 `wget`、回退到 `curl` 下载 GitHub `main` 分支压缩包。安装器不会调用软件包管理器；下载工具、dnsdist、WireGuard 工具、Python 3 等缺失时，它会列出缺失命令和建议的手动安装命令，然后退出。完整压缩包解压到安装目录后，再执行其中的完整安装器。
